...
void show_segmentos(uint8_t valor_hex);
...
void setup(void)
{
	...
	// PC1 - PC5 como salidas(segmentos y habilitador de digito)
...
uint8_t adc_read(void)
{
	...
	return ADCH;//Parte alta de la elctura
}
...
void multiplexar_displays(uint8_t valor)
{
	uint8_t high = (valor >> 4) & 0x0F;
	uint8_t low = valor & 0x0F;
